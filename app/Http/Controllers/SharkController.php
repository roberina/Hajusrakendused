...
class SharkController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        if ($request->hasFile('image')) {
            $data['image'] = '/storage/' . $request->file('image')->store('sharks', 'public');
        } else {
            $data['image'] = null;
        }
    ...
    public function api(Request $request)
    {
        ...
        $data = Cache::remember($cacheKey, now()->addMinutes(10), function () use ($search, $habitat, $danger, $sort, $order, $limit) {
            ...
            if ($habitat) $query->where('habitat', 'like', "%{$habitat}%");
            if ($danger)  $query->where('danger_level', $danger);
            return $query->orderBy($sort, $order)->limit($limit)->get();
        });

        return response()->json([
            'meta' => compact('limit', 'sort', 'order', 'search', 'habitat', 'danger'),
            'data' => $data,
        ]);
    }
}